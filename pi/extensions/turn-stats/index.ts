...
class StatsBlock implements Component {
	...
	render(width: number): string[] {
		...
		return [...rule, ...new Text(this.dim(this.stats), 1, 0).render(width)];
	}
	invalidate(): void {}
}
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerEntryRenderer<CompletionEntry>(ENTRY_TYPE, (entry: any, _options: any, theme: any) => {
		...
		// All-gray styling: assemble the whole line as plain text, then color it once
		// through the same dim function as the rule so nothing stands out.
		const groupSep = " · ";
		const groups: string[] = [];

		// Wall-clock finish time and elapsed duration as separate dot-joined groups.
		groups.push(formatClock(data.endedAt));
		groups.push(formatDuration(data.elapsedMs));
		...
		if (timing?.ttftMs !== undefined) groups.push(`ttft ${formatLatency(timing.ttftMs)}`);
		if (timing?.tokensPerSecond !== undefined) {
			groups.push(`tps ${formatTokensPerSecond(timing.tokensPerSecond).replace("/s", "")}`);
		}
		...
		if (usage && (usage.input > 0 || usage.output > 0)) {
			const bits: string[] = [];
			if (usage.input > 0) bits.push(`↓${formatTokensCompact(usage.input)}`);
			if (usage.output > 0) bits.push(`↑${formatTokensCompact(usage.output)}`);
			groups.push(bits.join(" "));
		}

		// Cache read/write volume and hit rate.
		if (usage && (usage.cacheRead > 0 || usage.cacheWrite > 0)) {
			groups.push(`cache r/w ${formatTokensCompact(usage.cacheRead)}/${formatTokensCompact(usage.cacheWrite)}`);
		}
		if (data.cacheHitPercent !== undefined) groups.push(`hit ${data.cacheHitPercent.toFixed(0)}%`);

		// ── Cost: run's marginal cost (the interesting per-turn metric) ───────────
		if (usage && usage.cost > 0) {
			groups.push(formatCostCents(usage.cost));
		}

		return new StatsBlock(
			groups.join(groupSep),
			// Uniform dim gray for the whole block (rule + stats), matching tps.ts.
